...
/**
 * Interface-only reservation demo form (TASK-066). No Server Action,
 * route handler, or network request exists — submitting only reveals
 * a demo-status notice. Native browser validation (via `required` /
 * `type`) handles field errors; no Zod or React Hook Form is used.
 */
function ReservationForm({ branches }: ReservationFormProps) {
  const formId = useId()
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <Textarea
          id={`${formId}-notes`}
          name="notes"
          placeholder="Occasion, seating preference, or anything else worth knowing"
        />
      </div>
